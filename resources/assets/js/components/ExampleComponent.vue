<template>
    <div class="row col-sm-12">
        <div id="page-head">
            ...
            <!--~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~-->
            <div id="page-title">
                <h1 class="page-header text-overflow">Thông tin người dùng</h1>
            </div>
            <!--~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~-->
            <!--End page title-->
        </div>
            ...
            <div class="panel">
                <div class="panel-body">
                    <div class="col-md-12">
                        <h2>
                            Test VueJS
                            <button class="btn btn-success" @click="showSingleModal()">
                        ...
                        <table class="table">
                            ...
                            <tbody>
                                <tr v-for="(item, key) in listExmaple" :key="key">
                                    ...
                                    <td>{{ item.address }}</td>
                                    <td>
                                        <a href="javascript:void(0)" class="btn btn-danger" @click="deleteExample(item.id)">
                                            <i class="fa fa-trash"></i>
                                        </a>
                                        <a href="javascript:void(0)" class="btn btn-default" @click="showSingleModal(item)">
...
export default {
    ...
    methods: {
        ...
        loadData: function() {
            var tmpThis = this;
            var param = service.exampleService.data.list(
                this.filter.freeText,
                this.filter.pagination.current_page,
                this.filter.pagination.per_page
            );
            service.exampleService.action
                .list(param)
                .then(respData => {
        ...
        deleteExample: function(id) {
            var tmpThis = this;
            myBootbox.confirm("Bạn có muốn xóa chuyên mục này không", function(
                result
            ) {
                if (result) {
                    service.exampleService.action
